...
class ImageTester:
    ...
    def fetch_wallets_json(self) -> List[Dict[str, Any]]:
        ...
        """
        url = f"{self.base_url}/wallets-v2.json"
        self.log(f"Fetching wallets data from: {url}")
        ...
        self.log(f"Successfully fetched {len(data)} wallet entries")
        return data
    ...
    def test_image_url(self, url: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if 'image/png' not in content_type:
                return False, f"Invalid content-type: {content_type}, expected image/png"
    ...
    def run_tests(self) -> Tuple[int, int, List[str]]:
        ...
        try:
            wallets = self.fetch_wallets_json()
            print(f"✓ Successfully fetched wallets-v2.json ({len(wallets)} wallets)")
        except Exception as e:
            print(f"✗ Failed to fetch wallets-v2.json: {e}")
            return 0, 1, [f"{self.base_url}/wallets-v2.json"]

        # Test 2: Extract and validate image URLs
        original_urls, test_urls = self.extract_image_urls(wallets)
        if not test_urls:
            print("✗ No image URLs found in wallets data")
            return 0, 1, []

        print(f"✓ Found {len(test_urls)} image URLs to test")
        ...
        for i, (original_url, test_url) in enumerate(zip(original_urls, test_urls), 1):
            ...
            if success:
                ...
                failed_urls.append(original_url)  # Report original URL in failures

            # Small delay to avoid overwhelming the server
            if i < len(test_urls):
                time.sleep(0.1)

        return passed, len(test_urls), failed_urls
...
def main() -> None:
    ...
    try:
        ...
        print("TEST RESULTS")
        print("=" * 60)
        print(f"Total tests: {total + 1}")  # +1 for wallets-v2.json fetch
        print(f"Passed: {passed + 1}")     # +1 for wallets-v2.json fetch
